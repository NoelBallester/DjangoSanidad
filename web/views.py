...
from .forms import (
    ...
    MuestraForm,
    MuestraHematologiaForm,
    MuestraNecropsiaForm,
    NecropsiaForm,
    NecropsiaInformeForm,
    TecnicoForm,
)
...
def _informes_por_registro(registro):
    ...
    return list(
        InformeResultado.objects.filter(
            content_type=ct,
...
def cassette_list(request):
    ...
    if sel_pk:
        try:
            ...
            try:
                ...
                if muestra_pk:
                    selected_muestra_item = next(
                        (
                            item
                            for item in muestras_con_imagenes
                            if str(item["muestra"].pk) == muestra_pk
                        ),
                        None,
                    )
            ...
            if informe_pk and informe_pk != "nuevo":
                informe_activo = next(
                    (item for item in informes_resultado if str(item.pk) == informe_pk),
                    None,
                )
    ...
    return render(
        ...
        "web/cassettes.html",
        {
            ...
            "cassette_form": CassetteForm(instance=selected)
            if selected
            else CassetteForm(),
            ...
            "todos_numeros": list(
                Cassette.objects.values_list("cassette", flat=True)
                .order_by("cassette")
                .distinct()
            ),
        },
...
def cassette_create(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            messages.error(
                request,
                "Error interno al guardar el cassette. Contacta con el administrador.",
            )
...
def _guardar_informe(
    request, pk, modelo, fk_campo, redirect_name, form_class=InformeForm
):
    ...
    if form.is_valid():
        ...
        return redirect(
            ...
            + f"?{fk_campo}={pk}&tab=informe&informe={informe.pk}"
        )
    messages.error(
        request,
        "No se pudo guardar el informe. Revisa los campos e inténtalo de nuevo.",
    )
...
def _eliminar_informe(request, pk, informe_pk, modelo, fk_campo, redirect_name):
    ...
    informe = get_object_or_404(
        InformeResultado, pk=informe_pk, content_type=ct, object_id=registro.pk
    )
    informe.delete()
...
def muestra_create(request, cassette_pk):
    ...
    if not (payload.get("descripcion") or "").strip():
        fallback_descripcion = (
            payload.get("numero_bloque")
            or payload.get("descripcion_macroscopica")
            or ""
        ).strip()
    ...
    if form.is_valid():
        ...
        return redirect(
            reverse("cassettes") + f"?cassette={cassette_pk}&muestra={muestra.pk}"
        )
...
def muestra_update(request, pk):
    ...
    if form.is_valid():
        form.save()
    return redirect(
        reverse("cassettes") + f"?cassette={muestra.cassette_id}&muestra={pk}"
    )
...
def imagen_upload(request, muestra_pk):
    ...
    if archivo_imagen:
        Imagen.objects.create(muestra=muestra, imagen=archivo_imagen.read())
    return redirect(
        reverse("cassettes") + f"?cassette={muestra.cassette_id}&muestra={muestra.pk}"
    )
...
def citologia_list(request):
    ...
    return render(
        ...
        "web/citologias.html",
        {
            ...
            "muestras_con_imagenes": muestras_con_imagenes,
            "selected_muestra_item": selected_muestra_item,
            "citologia_form": CitologiaForm(instance=selected)
            if selected
            else CitologiaForm(),
            ...
            "todos_numeros": list(
                Citologia.objects.values_list("citologia", flat=True)
                .order_by("citologia")
                .distinct()
            ),
        },
...
def qr_resolver(request):
    ...
    muestra_cassette = (
        Muestra.objects.filter(qr_muestra=payload).select_related("cassette").first()
    )
    if muestra_cassette:
        return redirect(
            reverse("cassettes")
            + f"?cassette={muestra_cassette.cassette_id}&muestra={muestra_cassette.pk}"
        )

    muestra_citologia = (
        MuestraCitologia.objects.filter(qr_muestra=payload)
        .select_related("citologia")
        .first()
    )
    if muestra_citologia:
        return redirect(
            reverse("citologias")
            + f"?citologia={muestra_citologia.citologia_id}&muestra={muestra_citologia.pk}"
        )

    muestra_necropsia = (
        MuestraNecropsia.objects.filter(qr_muestra=payload)
        .select_related("necropsia")
        .first()
    )
    if muestra_necropsia:
        return redirect(
            reverse("necropsias")
            + f"?necropsia={muestra_necropsia.necropsia_id}&muestra={muestra_necropsia.pk}"
        )
    ...
    muestra_hematologia = (
        MuestraHematologia.objects.filter(qr_muestra=payload)
        .select_related("hematologia")
        .first()
    )
    if muestra_hematologia:
        return redirect(
            reverse("hematologias")
            + f"?hematologia={muestra_hematologia.hematologia_id}&muestra={muestra_hematologia.pk}"
        )
    ...
    muestra_tubo = (
        MuestraTubo.objects.filter(qr_muestra=payload).select_related("tubo").first()
    )
    if muestra_tubo:
        return redirect(
            reverse("bioquimica")
            + f"?tubo={muestra_tubo.tubo_id}&muestra={muestra_tubo.pk}"
        )
    ...
    muestra_microbiologia = (
        MuestraMicrobiologia.objects.filter(qr_muestra=payload)
        .select_related("microbiologia")
        .first()
    )
    if muestra_microbiologia:
        return redirect(
            reverse("microbiologia")
            + f"?microbiologia={muestra_microbiologia.microbiologia_id}&muestra={muestra_microbiologia.pk}"
        )

    messages.error(
        request,
        "No se encontró ninguna muestra, citología, necropsia o cassette para ese QR.",
    )
    return redirect("cassettes")
...
def citologia_create(request):
    ...
    if form.is_valid():
        ...
        try:
            ...
            messages.error(
                request,
                "Error interno al guardar la citología. Contacta con el administrador.",
            )
...
def citologia_informe_delete(request, pk, informe_pk):
    return _eliminar_informe(
        request, pk, informe_pk, Citologia, "citologia", "citologias"
    )
...
def muestra_citologia_create(request, citologia_pk):
    ...
    if form.is_valid():
        ...
        archivo_imagen = request.FILES.get("imagen")
        if archivo_imagen:
            ImagenCitologia.objects.create(
                muestra=muestra, imagen=archivo_imagen.read()
            )
        return redirect(
            reverse("citologias") + f"?citologia={citologia_pk}&muestra={muestra.pk}"
        )
...
def muestra_citologia_update(request, pk):
    ...
    if form.is_valid():
        form.save()
    return redirect(
        reverse("citologias") + f"?citologia={muestra.citologia_id}&muestra={pk}"
    )
...
def imagen_citologia_upload(request, muestra_pk):
    ...
    return redirect(
        reverse("citologias")
        + f"?citologia={muestra.citologia_id}&muestra={muestra.pk}"
    )
...
def necropsia_list(request):
    ...
    return render(
        ...
        "web/necropsias.html",
        {
            ...
            "muestras_con_imagenes": muestras_con_imagenes,
            "selected_muestra_item": selected_muestra_item,
            "necropsia_form": NecropsiaForm(instance=selected)
            if selected
            else NecropsiaForm(),
            "nueva_necropsia_form": NecropsiaForm(),
            "muestra_form": MuestraNecropsiaForm(),
            "informe_form": NecropsiaInformeForm(initial=informe_initial)
            if selected
            else None,
            "informes_resultado": informes_resultado,
            "informe_activo": informe_activo,
            ...
            "todos_numeros": list(
                Necropsia.objects.values_list("necropsia", flat=True)
                .order_by("necropsia")
                .distinct()
            ),
        },
...
def necropsia_informe_delete(request, pk, informe_pk):
    return _eliminar_informe(
        request, pk, informe_pk, Necropsia, "necropsia", "necropsias"
    )
...
def muestra_necropsia_create(request, necropsia_pk):
    ...
    if form.is_valid():
        ...
        archivo_imagen = request.FILES.get("imagen")
        if archivo_imagen:
            ImagenNecropsia.objects.create(
                muestra=muestra, imagen=archivo_imagen.read()
            )
        return redirect(
            reverse("necropsias") + f"?necropsia={necropsia_pk}&muestra={muestra.pk}"
        )
...
def muestra_necropsia_update(request, pk):
    ...
    if form.is_valid():
        form.save()
    return redirect(
        reverse("necropsias") + f"?necropsia={muestra.necropsia_id}&muestra={pk}"
    )
...
def imagen_necropsia_upload(request, muestra_pk):
    ...
    return redirect(
        reverse("necropsias")
        + f"?necropsia={muestra.necropsia_id}&muestra={muestra.pk}"
    )
...
def hematologia_list(request):
    ...
    return render(
        ...
        "web/hematologias.html",
        {
            ...
            "informe_activo": informe_activo,
            "informe_imagen_base64": informe_imagen_base64,
            "hematologia_form": HematologiaForm(instance=selected)
            if selected
            else HematologiaForm(),
...
def hematologia_informe_delete(request, pk, informe_pk):
    return _eliminar_informe(
        request, pk, informe_pk, Hematologia, "hematologia", "hematologias"
    )
...
def imagen_hematologia_upload(request, muestra_pk):
    ...
    if form.is_valid():
        ...
        if img_file:
            ImagenHematologia.objects.create(muestra=muestra, imagen=img_file.read())
    return redirect(reverse("hematologias") + f"?hematologia={muestra.hematologia_id}")
...
def usuario_create(request):
    ...
    if request.method == "POST":
        ...
        if Tecnico.objects.filter(username=username).exists():
            messages.error(
                request, "Usuario: ya existe un técnico con ese nombre de usuario."
            )
            return redirect("usuarios")
...
def usuario_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if (
            username
            and Tecnico.objects.filter(username=username)
            .exclude(pk=tecnico.pk)
            .exists()
        ):
            messages.error(
                request, "Usuario: ya existe un técnico con ese nombre de usuario."
        ...
        if (
            email
            and Tecnico.objects.filter(email=email).exclude(pk=tecnico.pk).exists()
        ):
            messages.error(request, "Email: ya existe un técnico con ese email.")
            return redirect("usuarios")
...
def usuario_bulk_delete(request):
    ...
    messages.success(
        request,
        f"Limpieza completada: {count} usuarios eliminados. Se han conservado los usuarios con ID 1 y 2, y el siguiente ID empezará en 3.",
    )
    return redirect("usuarios")
...
def _descargar_volante(instancia):
    ...
    response["Content-Disposition"] = (
        f'inline; filename="{instancia.volante_peticion_nombre or "volante.pdf"}"'
    )
    return response
...
def descargar_informe_resultado(request, informe_pk):
    ...
    try:
        ...
        ext = (
            os.path.splitext(getattr(informe.imagen, "name", "") or "")[1].lstrip(".")
            or "bin"
        )
    except Exception:
        content = None
