...
def _validar_catalogo(tipo, valor, campo):
    ...
    if CatalogoOpcion.objects.filter(
        tipo=tipo, valor__iexact=valor_norm, activo=True
    ).exists():
        return valor_norm
...
class CassetteSerializer(
    QrUnicoValidatorMixin, FileUrlSerializerMixin, serializers.ModelSerializer
):
    qr_field = "qr_casette"
...
class CitologiaSerializer(
    ...
    def validate_tipo_citologia(self, value):
        return _validar_catalogo(
            CatalogoOpcion.TIPO_CITOLOGIA, value, "Tipo de citologia"
        )
...
class NecropsiaSerializer(
    ...
    def validate_tipo_necropsia(self, value):
        return _validar_catalogo(
            CatalogoOpcion.TIPO_AUTOPSIA, value, "Tipo de necropsia"
        )
...
class MuestraHematologiaSerializer(
    QrUnicoValidatorMixin, FileUrlSerializerMixin, serializers.ModelSerializer
):
    tincion = serializers.CharField(required=False, allow_blank=True)
    qr_muestra = serializers.CharField(required=False, allow_blank=True)
...
class MuestraMicrobiologiaSerializer(
    QrUnicoValidatorMixin, FileUrlSerializerMixin, serializers.ModelSerializer
):
    tincion = serializers.CharField(required=False, allow_blank=True)
    qr_muestra = serializers.CharField(required=False, allow_blank=True)
...
class ImagenMicrobiologiaSerializer(
    FileUrlSerializerMixin, serializers.ModelSerializer
):
    imagen_url = serializers.SerializerMethodField()
...
class InformeResultadoSerializer(FileUrlSerializerMixin, serializers.ModelSerializer):
    ...
    def validate_tincion(self, value):
        return _validar_catalogo("tincion", value, "tincion")
    ...
    def validate(self, attrs):
        ...
        if self.instance is None and len(provided) != 1:
            raise serializers.ValidationError(
                "Debe indicar exactamente un destino para el informe."
            )
        ...
        if provided:
            ...
            try:
                ...
                raise serializers.ValidationError(
                    {field: "Registro de destino no encontrado."}
                )
        ...
        if isinstance(imagen_data, str):
            if not imagen_data.strip():
                ...
                try:
                    ...
                    raise serializers.ValidationError(
                        {"imagen": "La imagen no es base64 válido."}
                    )

                mutable["imagen"] = ContentFile(
                    raw, name=f"informe_{uuid.uuid4().hex}{extension}"
                )
            else:
                ...
                try:
                    ...
                    mutable["imagen"] = ContentFile(
                        raw, name=f"informe_{uuid.uuid4().hex}.bin"
                    )
                except (ValueError, binascii.Error):
                    raise serializers.ValidationError(
                        {"imagen": "La imagen no es base64 válido."}
    ...
    def create(self, validated_data):
        ...
        informe = InformeResultado.objects.create(
            content_object=target, **validated_data
        )
        return informe
