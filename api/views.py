...
from .serializers import (
    CassetteSerializer,
    CitologiaSerializer,
    HematologiaSerializer,
    ImagenCitologiaSerializer,
    ImagenHematologiaSerializer,
    ImagenMicrobiologiaSerializer,
    ImagenNecropsiaSerializer,
    ImagenSerializer,
    ImagenTuboSerializer,
    InformeResultadoSerializer,
    MicrobiologiaSerializer,
    MuestraCitologiaSerializer,
    MuestraHematologiaSerializer,
    MuestraMicrobiologiaSerializer,
    MuestraNecropsiaSerializer,
    MuestraSerializer,
    MuestraTuboSerializer,
    NecropsiaSerializer,
    TecnicoSerializer,
    TuboSerializer,
)
...
def _validar_imagen_api(imagen_file):
    ...
    es_imagen = (
        ...
        or cabecera.startswith((b"GIF87a", b"GIF89a"))
        or cabecera.startswith(b"BM")
        or es_webp
        or cabecera.startswith((b"II*\x00", b"MM\x00*"))
    )
    if not es_imagen:
...
def _roles_permitidos_para_proxy(model, instance=None):
    ...
    if model is InformeResultado and instance is not None:
        target_model = (
            instance.content_type.model_class() if instance.content_type_id else None
        )
...
class ActualizarInformeMixin:
    ...
    def actualizar_informe(self, request, pk=None):
        ...
        if "informe_imagen" in data:
            ...
            instance.informe_imagen = (
                base64.b64decode(imagen_data)
...
class RegistroViewSet(ActualizarInformeMixin, viewsets.ModelViewSet):
    ...
    def rango_fechas(self, request):
        ...
        if not fecha_inicio or not fecha_fin:
            return Response(
                {"error": "Se requieren inicio y fin"},
...
class TecnicoViewSet(viewsets.ModelViewSet):
    ...
    def get_by_mail(self, request, mail=None):
        ...
        if not tecnico:
            return Response(
                {"error": "Técnico no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
...
class MuestraCitologiaViewSet(MuestraSoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        # Generar QR automáticamente si no existe
        if "qr_muestra" not in data or not data["qr_muestra"]:
            data["qr_muestra"] = generar_qr_unico(
                "--mc--", MuestraCitologia, "qr_muestra"
            )
        serializer = self.get_serializer(data=data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
...
class MuestraNecropsiaViewSet(MuestraSoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if "qr_muestra" not in data or not data["qr_muestra"]:
            data["qr_muestra"] = generar_qr_unico(
                "--mn--", MuestraNecropsia, "qr_muestra"
            )
        serializer = self.get_serializer(data=data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
...
class ImagenViewSet(SoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if not muestra_id:
            return Response(
                {"error": "No se proporcionó ID de muestra"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"error": _extract_validation_error_message(exc)},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class ImagenTuboViewSet(SoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        return Response(
            self.get_serializer(imagen_tubo).data, status=status.HTTP_201_CREATED
        )
...
class MuestraHematologiaViewSet(MuestraSoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if "qr_muestra" not in data or not data["qr_muestra"]:
            data["qr_muestra"] = generar_qr_unico(
                "--mh--", MuestraHematologia, "qr_muestra"
            )
...
class ImagenHematologiaViewSet(SoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        return Response(
            self.get_serializer(imagen_hematologia).data, status=status.HTTP_201_CREATED
        )
...
class MicrobiologiaViewSet(RegistroViewSet):
    ...
    def create(self, request):
        ...
        if not data.get("qr_microbiologia"):
            data["qr_microbiologia"] = generar_qr_unico(
                self.qr_prefix, Microbiologia, self.qr_field
            )
        serializer = self.get_serializer(data=data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
    ...
    def por_organo(self, request, organo=None):
        ...
        if organo == "*":
            ...
            microbiologias = Microbiologia.objects.filter(organo=organo).order_by(
                "-fecha"
            )
        return Response(MicrobiologiaSerializer(microbiologias, many=True).data)
    ...
    def por_numero(self, request, numero=None):
        """Filtra por número"""
        microbiologias = Microbiologia.objects.filter(microbiologia=numero).order_by(
            "-fecha"
        )
        return Response(MicrobiologiaSerializer(microbiologias, many=True).data)
...
class MuestraMicrobiologiaViewSet(MuestraSoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if "qr_muestra" not in data or not data["qr_muestra"]:
            data["qr_muestra"] = generar_qr_unico(
                "--mmb--", MuestraMicrobiologia, "qr_muestra"
            )
...
class ImagenMicrobiologiaViewSet(SoftDeleteDestroyMixin, viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        return Response(
            self.get_serializer(imagen_microbiologia).data,
...
class InformeResultadoViewSet(viewsets.ModelViewSet):
    ...
    def _filtrar_por_modelo(modelo, object_id):
        ...
        return InformeResultado.objects.filter(
            content_type=ct,
            object_id=object_id,
        ).order_by("-creado_en")
    ...
    def create(self, request):
        ...
        return Response(
            self.get_serializer(informe).data, status=status.HTTP_201_CREATED
        )
