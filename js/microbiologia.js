...
const configurarEventosInformesMicrobiologia = () => {
  if (!informesListaMicrobiologia) return;
  if (informesListaMicrobiologia.dataset.informeEventsBound === "1") return;
  informesListaMicrobiologia.dataset.informeEventsBound = "1";
...
configurarEventosInformesMicrobiologia();
...
const obtenerUrlInformeMicrobiologia = (informe) => {
  if (!informe) return "";
...
window.verInformeMicrobiologia = (url) => {
  if (!url) {
    mostrarEstadoInforme("Este informe no tiene archivo adjunto.", "warning");
    return;
  }
...
window.eliminarInformeMicrobiologia = async (informeId) => {
  const targetId = currentMicrobiologiaId || microbiologiaId;
  if (!targetId || !informeId) return;
  if (!confirm("¿Eliminar este informe?")) return;
...
const imprimirInformesMicrobiologia = (informes) => {
  ...
  informes.forEach((informe) => {
    const urlInforme = obtenerUrlInformeMicrobiologia(informe);
    const tieneArchivo = Boolean(obtenerUrlInformeMicrobiologia(informe));
...
const imprimirMuestras = (respuesta) => {
  ...
  if (respuesta.length > 0) {
    respuesta.forEach((muestra) => {
      ...
      let btndetalle = document.createElement("I");
      btndetalle.className =
        "d-inline-block microbiologia__icon fa-solid fa-eye microbiologia__icon--infomicrobiologia icono__efect me-2";
      ...
      btneditar.className =
        "d-inline-block microbiologia__icon fa-solid fa-file-pen microbiologia__icon--infomicrobiologia icono__efect me-2";
